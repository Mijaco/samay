 ...
 * 
 * Resumen.
 * Objeto : BandejaCaudalesController
 * Descripción : Clase controladora para la funcionalidad de Caudales
...
public class BandejaCaudalesController {
	...
	/**
	 * Método que permite redirigir a la vista de Bandeja de Caudales
	 ...
	 **/
	@RequestMapping(value = "/bandejaCaudales", method = RequestMethod.GET)
	public String bandejaCaudalesGo(ModelMap model) throws Exception {
		List<SelectItemBean> riosSelect = mantRioService.obtenerRiosTodos();
		model.addAttribute("riosAutocomplete", riosSelect);
		model.addAttribute("ultimaCarga", cargaService.obtenerUltimaCarga());
		return "bandejaCaudales";
	}
	 ...
	 * @throws Exception, Excepción que puede ser lanzada
	 **/
	@RequestMapping(value = "/bandejaCaudales/pagination", method = RequestMethod.POST)
	public @ResponseBody ResponseEntity<Map<String,Object>> obtenerDatos(@RequestParam Map<String,String> allRequestParams)
		...
		try {
			...
			if(allRequestParams.get("nombreLargo")!=null && !"".equals(allRequestParams.get("nombreLargo"))){
				caudalSearchBean.setStrNombreLargo(allRequestParams.get("nombreLargo").toUpperCase());
			}
			...
			Result result = caudalService.buscarCaudales(caudalSearchBean, paginacion);
