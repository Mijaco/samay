 ...
 * 
 * Resumen.
 * Objeto : RepresaDaoImpl
 * Descripción : Clase de implementación de métodos de persistencia para el mantenimiento de Represa
...
public class RepresaDaoImpl implements IMantRepresaDao {
	...
	public void inactivarRepresa(RepresaBean represaBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), CoreUtils.concatenar(Constants.PACKAGE_REPRESA,ConstantsCommon.P_SEPARADOR,Constants.SP_INACTIVA_MANTREPRESA), paramsInput, paramsOutput);
	...
	public void grabarRepresa(RepresaBean represaBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), 
					CoreUtils.concatenar(Constants.PACKAGE_REPRESA,ConstantsCommon.P_SEPARADOR,Constants.SP_INSERT_REPRESA), lstParamsInput, lstParamsOutput);
			inputs = new HashMap<String, Object>();
	...
	public void actualizarRepresa(RepresaBean represaBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), Constants.PACKAGE_REPRESA+Constants.P_SEPARADOR+Constants.SP_UPDATE_REPRESA, paramsInput, paramsOutput);
			inputs = new HashMap<String, Object>();
			inputs.put(Constants.PAR_NCODIGO_REPRESA, represaBean.getCodigo());
			inputs.put(Constants.PAR_VNOMBRE_REPRESA, represaBean.getNombreRepresa());
			inputs.put(Constants.PAR_VABREVIATURA_REPRESA, represaBean.getAbreviatura());
