...
public class AnalisisHibroBiologicoDaoImpl implements IAnalisisHibroBiologicoDao {
	...
	public void insertarAnalisisHidroBiologico(AnalisisHidroBiologico analisisHidroBiologico) {
		...
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_FECANA, OracleTypes.DATE));
		...
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_CANTID, OracleTypes.INTEGER));
		...
		this.execSp = new ExecuteProcedure(template.getDataSource(), ConstantsLaboratorio.PCK_ALC_ANALISISHIDROBIO
				+ Constants.P_SEPARADOR + ConstantsLaboratorio.PRC_INSERT_ANALISISHIDROBIO, paramsInput, paramsOutput);
		inputs = new HashMap<>();
		...
		inputs.put(ConstantsLaboratorio.PAR_FECANA, Utils.getDateOracle(analisisHidroBiologico.getFechaAnalisis()));
		...
		inputs.put(ConstantsLaboratorio.PAR_CANTID, analisisHidroBiologico.getCantidad());
...
//											N_CODREP, D_FECREP, D_FECMUE, N_PERMUE, N_PERANA 
	...
	public ReporteAnalisisHidroDetalle datosDetalleReporteHidroBiologico(String fechaAnalisis, Integer puntoMuestreo) {
		...
		for (Object object : listaDetalles) {
			...
			if(object instanceof List){
				List<Object> detalle = (List<Object>)object;
				Object llave = detalle.get(0);
				Object valor = detalle.get(1);
				
					if(idAlgas.equals(llave)){
						reporteAnalisisHidroDetalle.setAlgas(Integer.parseInt(valor.toString()));
					}else if (idCianobacterias.equals(llave)){
						reporteAnalisisHidroDetalle.setCianobacterias(Integer.parseInt(valor.toString()));
					}else if (idNematodes.equals(llave)){
						reporteAnalisisHidroDetalle.setNematodes(Integer.parseInt(valor.toString()));
					}else if (idOtros.equals(llave)){
						reporteAnalisisHidroDetalle.setOtros(Integer.parseInt(valor.toString()));
					}
					
					totalOrg += Integer.parseInt(valor.toString());
			}
		}
	...
	public void insertarReporteHidroBiologico(ReporteAnalisisHidroDetalle reporteAnalisisHidroDetalle) {
		...
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_CODSUB, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_CANALG, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_CIANOB, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_NEMATO, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_ZOTROS, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_TOTORG, OracleTypes.INTEGER));
		paramsInput.add(new SqlParameter(ConstantsLaboratorio.PAR_ECAMIN, OracleTypes.INTEGER));
		
		paramsOutput = new ArrayList<>();

		this.execSp = new ExecuteProcedure(template.getDataSource(), ConstantsLaboratorio.PCK_ALC_ANALISISHIDROBIO
				+ Constants.P_SEPARADOR + ConstantsLaboratorio.PRC_INSERT_REPORTEHIDROBIO, paramsInput, paramsOutput);
		inputs = new HashMap<>();
						...
						new SqlOutParameter(Constants.PAR_OUTQUANTITY, Types.INTEGER), new SqlOutParameter(
								ConstantsCommon.PAR_OUT_CURSOR, OracleTypes.CURSOR, new RowMapper<ReporteAnalisisHidroDetalle>() {
									...
									public ReporteAnalisisHidroDetalle mapRow(ResultSet rs, int rowNum) throws SQLException {
										...
										reporteAnalisisHidroDetalle.setFechaMuestreoDate(rs.getDate(2));
										...
										reporteAnalisisHidroDetalle.setAlgas(rs.getInt(7));
										reporteAnalisisHidroDetalle.setCianobacterias(rs.getInt(8));
										reporteAnalisisHidroDetalle.setNematodes(rs.getInt(9));
										reporteAnalisisHidroDetalle.setOtros(rs.getInt(10));
										reporteAnalisisHidroDetalle.setTotal(rs.getInt(11));
										reporteAnalisisHidroDetalle.setMinam(rs.getInt(12));
	...
	public boolean eliminarReporteHidroPorCorrelativo(Integer idCorrelativo){
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), ConstantsLaboratorio.PCK_ALC_ANALISISHIDROBIO
					+ Constants.P_SEPARADOR + ConstantsLaboratorio.PRC_DELETE_REPORTEHIDRO, paramsInput, paramsOutput);
			inputs = new HashMap<>();
						...
						new SqlOutParameter(Constants.PAR_OUTQUANTITY, Types.INTEGER), new SqlOutParameter(
								ConstantsCommon.PAR_OUT_CURSOR, OracleTypes.CURSOR, new RowMapper<AnalisisHidroBiologico>() {
									...
									public AnalisisHidroBiologico mapRow(ResultSet rs, int rowNum) throws SQLException {
										...
										analisisHidroBiologico.setCantidad(rs.getInt(6));
