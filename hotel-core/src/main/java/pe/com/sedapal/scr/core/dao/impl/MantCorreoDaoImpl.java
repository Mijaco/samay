 ...
 * 
 * Resumen.
 * Objeto : MantCorreoDaoImpl
 * Descripción : Clase de implementación de métodos de persistencia para el mantenimiento de Configuración de Correo
...
public class MantCorreoDaoImpl implements IMantCorreoDao {
	...
	public void grabarMantCorreo(MantCorreoEditBean mantCorreoEditBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), 
					CoreUtils.concatenar(Constants.PACKAGE_CORREO,ConstantsCommon.P_SEPARADOR,Constants.SP_INSERTA_MANTCORREO), lstParamsInput, lstParamsOutput);
	...
	public void actualizarMantCorreo(MantCorreoEditBean mantCorreoEditBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), 
					CoreUtils.concatenar(Constants.PACKAGE_CORREO,ConstantsCommon.P_SEPARADOR,Constants.SP_UPDATE_MANTCORREO), lstParamsInput, lstParamsOutput);
			inputs = new HashMap<String, Object>();
	...
	public void inactivarCorreo(MantCorreoBean mantCorreoBean) throws Exception {
		...
		try {
			...
			this.execSp = new ExecuteProcedure(template.getDataSource(), CoreUtils.concatenar(Constants.PACKAGE_CORREO,ConstantsCommon.P_SEPARADOR,Constants.SP_INACTIVA_MANTCORREO), paramsInput, paramsOutput);
			inputs = new HashMap<String, Object>();
